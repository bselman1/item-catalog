...
class GoogleAuthCallback(MethodView):
    ...
    # The function that will be redirected to by the google authentication
    # servers
    def get(self):
        # Rebuild the flow from the original request using the state stored in
        # the session
...
def add_routes(app: Flask, google_auth: GoogleAuthConfig):
    ...
    # The URL is pre-registered with google and is tied to this application's
    # client_id. We'll use the GoogleAuthCallback class as a view that google
    # can redirect to.
    app.add_url_rule(
        '/oauth2callback',
        view_func=GoogleAuthCallback.as_view(
            'google_callback',
            google_auth=google_auth))

    app.add_url_rule(
        '/login',
        view_func=GoogleLoginView.as_view(
            'google_login_view',
            google_auth=google_auth))
