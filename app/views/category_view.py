...
def add_routes(app: Flask, catalog_service: CatalogService):
    app.add_url_rule(
        '/categories',
        view_func=CategoriesView.as_view(
            'categories_view',
            catalog_service=catalog_service))

    app.add_url_rule(
        '/api/categories',
        view_func=CategoriesViewJson.as_view(
    ...
    app.add_url_rule(
        ...
        view_func=CategoryViewJson.as_view(
            'category_json_view',
            catalog_service=catalog_service))

    app.add_url_rule(
        '/api/catalog',
        view_func=CatalogViewJson.as_view(
            'catalog_json_view',
            catalog_service=catalog_service))
...
class CatalogViewJson(MethodView):
    ...
    def get(self):
        ...
        category_dicts = {
            'categories': [category.to_dict() for category in catalog]
        }
        return jsonify(category_dicts)
